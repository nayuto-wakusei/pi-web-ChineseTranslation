...
describe("Docker command assets", () => {
  ...
  it("keeps shell entrypoints syntactically valid", async () => {
    await Promise.all([
      execUtf8(shellExecutable, ["-n", dockerEntrypoint], process.env),
      execUtf8(shellExecutable, ["-n", join(repoRoot, "docker", "install.sh")], process.env),
      execUtf8(shellExecutable, ["-n", join(repoRoot, "docker", "internal", "dev", "compose")], process.env),
      execUtf8(shellExecutable, ["-n", join(repoRoot, "docker", "internal", "host-profile.sh")], process.env),
    ]);
  });
...
function resolveShellExecutable(): string {
  ...
  const pathEntries = (process.env["Path"] ?? process.env["PATH"] ?? "").split(delimiter).filter((entry) => entry !== "");
  const pathCandidates = pathEntries.flatMap((entry) => [join(entry, "sh.exe"), join(dirname(entry), "bin", "sh.exe"), join(dirname(entry), "usr", "bin", "sh.exe")]);
  const candidates = [...pathCandidates, join(programFiles, "Git", "bin", "sh.exe"), join(programFiles, "Git", "usr", "bin", "sh.exe")];
