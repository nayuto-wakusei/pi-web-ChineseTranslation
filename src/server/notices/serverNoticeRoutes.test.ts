...
describe("server notice routes", () => {
  it.each(["GET", "POST"] as const)("rejects management %s requests without exposing or dismissing normal notices", async (method) => {
    const notice = service.record({ severity: "error", message: "Private workspace failure", source: "workspace.delete" });
    const before = service.snapshot();
    const context = encodeManagementContext({
      user: { id: "managed-user", rootUserId: "managed-root", roles: [], permissions: [] },
      projects: [{ id: "managed-project", name: "Managed project", root: process.cwd() }],
    });

    const response = await app.inject({
      method,
      url: method === "GET" ? "/notices" : "/notices/dismiss",
      headers: { [MANAGEMENT_EMBED_CONTEXT_HEADER]: context },
      ...(method === "POST" ? { payload: { daemonInstanceId: "daemon-a", noticeId: notice.id } } : {}),
    });

    expect(response.statusCode).toBe(403);
    expect(response.json()).toEqual({ error: "Server notices are unavailable in management embed mode" });
    expect(response.body).not.toContain(notice.message);
