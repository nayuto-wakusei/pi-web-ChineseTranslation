...
export type WorkspaceCatalogInput = WorkspaceCatalog | WorkspaceService;

export function asWorkspaceCatalog(projects: WorkspaceCatalogProjectReader, workspaces: WorkspaceCatalogInput): WorkspaceCatalog {
  if (isWorkspaceCatalog(workspaces)) return workspaces;
  return {
    resolveProject: async (projectId) => {
      ...
      return { status: "folder", projectId, workspaces: listed, diagnostics: [] };
    },
    list: async (projectId) => {
      const project = await projects.requireProject(projectId);
      return await workspaces.list(project);
    },
    resolve: async (projectId, workspaceId) => {
      const project = await projects.requireProject(projectId);
...
function isWorkspaceCatalog(value: WorkspaceCatalogInput): value is WorkspaceCatalog {
  return "resolveProject" in value && "resolve" in value;
}
