...
import { friendlyPiPackageErrorMessage, isPiPackageManagementUnsupported, piPackageManagementSupport, piPackageManagementSupportKey, piPackageMutationFollowUpMessage, piPackagesResponseAfterMutation, piPackageTargetLabel, shouldRefreshGatewayPluginsAfterPiPackageMutation, type PiPackageManagementSupport, type PiPackageOperationState, type PiPackageTargetContext } from "./settings/piPackageSettings";
...
export class SettingsDialog extends LitElement {
  ...
  override render(): TemplateResult {
    return html`
      <modal-surface .onClose=${() => this.onClose?.()} .busy=${this.saving} .label=${"PI WEB 设置"}>
          ...
          <div class="settings-body">
            <nav class="settings-nav" aria-label="设置分区">
              ${this.renderNavButton("general", "通用", "网关与所选机器")}
              ${this.renderNavButton("sessiond", "会话守护进程", "所选机器")}
              ${this.renderNavButton("packages", "Pi 包", "所选机器")}
              ${this.renderNavButton("plugins", "PI WEB 插件", "所选机器")}
              ${this.renderNavButton("shortcuts", "键盘", "网关快捷键")}
            </nav>
  ...
  private async refreshGatewayPlugins(): Promise<string | undefined> {
    try {
      ...
      return `刷新网关 PI WEB 插件失败：${errorMessage(error)}`;
    }
  }
  ...
  static override styles = css`
    ...
    @media (max-width: 760px) {
      modal-surface { --modal-surface-place-items: stretch; --modal-surface-backdrop-padding: 0; --modal-surface-width: 100%; --modal-surface-height: 100dvh; --modal-surface-max-height: none; --modal-surface-min-height: 0; --modal-surface-border: 0; --modal-surface-radius: 0; }
