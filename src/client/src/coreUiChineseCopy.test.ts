...
const copyCases: readonly CopyCase[] = [
  ...
  },
  {
    ...
    paths: ["components/ChatView.ts", "controllers/sessionController.ts", "sessionDaemonError.ts"],
    expected: ["activityStateLabel(activity.label, activity.phase)", "return activityStateLabel(state);", "空闲", "正在接收回复", "正在运行工具", "工具执行完成", "正在重新加载资源", "running: \"运行中\"", "正在创建会话", "会话创建失败", "启动会话失败", "会话守护进程请求超时"],
    forbidden: [
