...
export class MachineController {
  ...
  async loadMachines(routeMachineId?: string): Promise<void> {
    ...
    try {
      ...
      this.setState({ machines, selectedMachine, machineActivities: filterKeys(this.getState().machineActivities, machineIds), machineRuntimes: filterKeys(this.getState().machineRuntimes, machineIds), machineStatusSnapshots: filterKeys(this.getState().machineStatusSnapshots, machineIds) });
  ...
  async deleteMachine(machine: Machine | undefined = this.getState().selectedMachine, options: { selectFallback?: boolean } = {}): Promise<Machine | undefined> {
    ...
    if (machine.kind === "local") {
      this.browserErrors.report(machineBrowserErrorScope(machine.id), "无法移除本地机器。");
      return undefined;
    }
    try {
  ...
  private async selectInitialMachine(machines: Machine[], routeMachineId?: string): Promise<Machine | undefined> {
    ...
    this.setState({ machineStatuses: { ...this.getState().machineStatuses, [health.machineId]: health } });
    if (!health.ok) this.browserErrors.report(machineBrowserErrorScope(requestedMachine.id), `${requestedMachine.name} 暂不可用，正在重新连接…`);
