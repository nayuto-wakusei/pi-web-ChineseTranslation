...
- Management startup force-syncs the bundled Relay template to `.pi/skills/relay/SKILL.md`, replacing existing content only within the owning workspace. Normal startup must neither create nor overwrite project skills. Suppress a Relay collision only when the project copy and a known bundled copy have identical content; retain unrelated diagnostics and management resource isolation.
