...
- Session preference defaults (`defaultThinkingLevel`, default model, and other Pi `SettingsManager` global settings written by the agent session) are scoped by mode and project under `$PI_WEB_DATA_DIR`: normal mode co-locates `settings.json` with the project auth store; management embed uses `management-embed/projects/<hash>/settings.json` (or `management-embed/orphan/<cwdHash>/` when cwd is not a unique registered project). Bootstrap copies the global agent `settings.json` once. Do not write these defaults back into the shared global agent dir from PI WEB session runtimes.
