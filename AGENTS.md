...
- Management-embed sessions may use the managed file tools and the custom `python` tool. Python must remain workspace-only and isolated through Bubblewrap; do not replace it with the upstream generic Python or shell execution path. Keep Python network-disabled unless a signed token `privileged.network` grant is in effect and `managementEmbed.allowPrivileged` is enabled on the machine.
- Management startup force-syncs both bundled `relay` and `relay-runner` templates to `.pi/skills/<name>/SKILL.md`, replacing existing content only within the owning workspace. Ship both templates in the npm package and retain both in the Workbench skill allowlist. Normal startup must neither create nor overwrite project skills. Suppress a collision for either Relay skill only when the project copy and a known bundled copy have identical content; retain unrelated diagnostics and management resource isolation.
- Generic `bash`, shell, PowerShell, terminal, MCP, HTTP, web-fetch, and web-search tools remain denied to the management agent by default. A signed token may request `privileged.bash` and/or `privileged.network`; those grants take effect only when `managementEmbed.allowPrivileged` is true. Privileged bash registers a constrained Bubblewrap `bash` tool: the current workspace is writable, other host paths are read-only, and the upstream SDK shell, PowerShell, and interactive terminals stay denied. Privileged network enables `http`, `webfetch`, and `websearch`, and also drops `--unshare-net` from the managed Python, bash, and `terminal-command-runs` sandboxes. Explicit `tools.deny` remains authoritative. Generic MCP stays denied. Python stays workspace-only.
...
Keep Pi SDK peer ranges within the tested minor series. Validate a clean installed package by starting both the web gateway and session daemon, not only the CLI version or terminal module; upstream SDK entrypoint imports may require runtime dependencies absent from its published manifest.
